                                            ...
                                            <div class="form-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            <label class="control-label">
                                                                {{__('order::dashboard.orders.datatable.notes')}}
                                                            </label>
                                                            <select name="note_id"
                                                                id="single"
                                                                ...
                                                                @inject('notes','Modules\Course\Entities\Note')
                                                                @foreach ($notes->get() as $note)
                                                                <option value="{{ $note['id'] }}"
                                                                    @if(request('note_id')==$note['id'])
                                                                    selected
                                                                    @endif>
                                                                    {{ $note->title }}
                                                                </option>
...
<script>
    function tableGenerate(data = '') {
        ...
        var dataTable =
            $('#dataTable').DataTable({
                ajax: {
                    url: "{{ url(route('dashboard.orders.datatable')) }}?order_type=note",
                    type: "GET",
                    data: {
                        req: data,
                    },
