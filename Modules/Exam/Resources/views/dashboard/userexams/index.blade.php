...
@section('title', __('exam::dashboard.userexams.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('exam::dashboard.userexams.routes.index')}}</a>
                </li>
            </ul>
        </div>
                                            ...
                                            <div class="form-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-3">
                                                      <div class="form-group">
                                                        <label class="control-label">
                                                          {{__('exam::dashboard.userexams.datatable.user')}}
                                                        </label>
                                                        <select name="user_id"
                                                                id="single"
                                                          ...
                                                          @foreach ($users->doesnthave('roles')->get() as $user)
                                                    ...
                                                    <div class="col-md-3">
                                                      <div class="form-group">
                                                        <label class="control-label">
                                                          {{__('exam::dashboard.userexams.datatable.exam')}}
                                                        </label>
                                                        <select name="exam_id"
                                                                id="single"
                                                                class="form-control select2">
                        ...
                        <table class="table table-striped table-bordered table-hover"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.user')}}</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.exam')}}</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.course')}}</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.trainer')}}</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.created_at')}}</th>
                                    <th>{{__('exam::dashboard.userexams.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit"
                                ...
                                onclick="deleteAllChecked('{{ url(route('dashboard.userexams.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.userexams.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            ...
            {data: 'exam_id' 	      , className: 'dt-center'},
            {data: 'course' 	      , className: 'dt-center', orderable: false},
            {data: 'trainer' 	      , className: 'dt-center', orderable: false},
...
responsivePriority:1,
              width: '13%',
              title: '{{__('exam::dashboard.userexams.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var showUrl = '{{ route("dashboard.userexams.show", ":id") }}';
      					showUrl = showUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.userexams.destroy", ":id") }}';
