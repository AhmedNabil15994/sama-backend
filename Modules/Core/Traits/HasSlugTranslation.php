...
trait HasSlugTranslation
{
    /**
     * Register the saving event callback
     */
    public static function bootHasSlugTranslation()
    {
    ...
    public function translateSlugs($attribute)
    {
        ...
        $value = $this->getTranslations($sluggable);
        if (!$this->isDirty($sluggable)) {
    ...
    public function getUniqueSlug($sluggable, $locale = null)
    {
        ...
        $slug = $locale == "ar" ? slugAr($sluggable) : Str::slug($sluggable, "-", $locale);
    ...
    public function getTranslatedSlugRouteParameters(Route $route, $locale = null)
    {
        ...
        $parameter = array_reduce($parameters, function ($carry, $parameter) {
            if ($carry || $parameter->getType()->getName() !== get_class()) {
                return $carry;
            }
            return $parameter;
        });
    ...
    protected function onlyCreated()
    {
        return $this->slugOnlyCreated ?? false;
    }
}
