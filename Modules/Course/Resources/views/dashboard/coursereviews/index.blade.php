...
@section('title', __('course::dashboard.coursereviews.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('course::dashboard.coursereviews.routes.index')}}</a>
                </li>
            </ul>
        </div>
                        ...
                        <table class="table table-striped table-bordered table-hover"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>{{__('course::dashboard.coursereviews.datatable.stars')}}</th>
                                    <th>{{__('course::dashboard.coursereviews.datatable.status')}}</th>
                                    <th>{{__('course::dashboard.coursereviews.datatable.created_at')}}</th>
                                    <th>{{__('course::dashboard.coursereviews.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            ...
                            <button type="submit"
                                ...
                                onclick="deleteAllChecked('{{ url(route('dashboard.coursereviews.deletes')) }}')">
...
<script>
    function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.coursereviews.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columns: [
            {data: 'id' 		 	       , className: 'dt-center'},
  			...
  			{data: 'stars' 			       , className: 'dt-center'},
...
responsivePriority:1,
              width: '13%',
              title: '{{__('course::dashboard.coursereviews.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.coursereviews.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.coursereviews.destroy", ":id") }}';
      					deleteUrl = deleteUrl.replace(':id', data);

      					return `
                @can('edit_coursereviews')
      						<a href="`+editUrl+`" class="btn btn-sm blue" title="Edit">
      			              <i class="fa fa-edit"></i>
      			            </a>
      					@endcan

                @can('delete_coursereviews')
                @csrf
