  ...
  <div class="page-content">
    <div class="page-bar">
      <ul class="page-breadcrumb">
        ...
        <li>
          <a href="#">{{__('apps::dashboard._layout.aside.review_questions')}}</a>
        </li>
      </ul>
    </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="portlet light bordered">
          ...
          @can('add_reviewquestions')
            <div class="table-toolbar">
            <div class="row">
              <div class="col-md-6">
                <div class="btn-group">
                  <a href="{{ url(route('trainer.reviewquestions.create')) }}" class="btn sbold green">
            ...
            <table class="table table-striped table-bordered table-hover" id="dataTable">
              <thead>
                <tr>
                  ...
                  <th>#</th>
                  <th>{{__('course::dashboard.reviewquestions.datatable.title')}}</th>
                  <th>{{__('course::dashboard.coursereviews.datatable.course')}}</th>
                  <th>{{__('course::dashboard.coursereviews.datatable.user')}}</th>
                  <th>{{__('course::dashboard.reviewquestions.datatable.created_at')}}</th>
                  <th>{{__('course::dashboard.reviewquestions.datatable.options')}}</th>
                </tr>
              </thead>
            </table>
          </div>
...
{{--                onclick="deleteAllChecked('{{ url(route('trainer.reviewquestions.deletes')) }}')">--}}
...
<script>
  function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ajax : {
              url   : "{{ url(route('trainer.reviewquestions.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
  			...
  			},
            {
              ...
              width: '13%',
              title: '{{__('course::dashboard.reviewquestions.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {
                // Edit
				var editUrl = '{{ route("trainer.reviewquestions.edit", ":id") }}';
				editUrl = editUrl.replace(':id', data);

				var goto = '{{ route("frontend.courses.show", ":id") }}' + '?lesson-content-id=' + full.lesson_content_id;
				goto = goto.replace(':id', full.course_slug);

				// Delete
				var deleteUrl = '{{ route("trainer.reviewquestions.destroy", ":id") }}';
				deleteUrl = deleteUrl.replace(':id', data);

				return `
                @can('edit_review_questions')
      						<a href="`+editUrl+`" class="btn btn-sm yellow" title="Edit">
      			              <i class="fa fa-reply"></i>
      			            </a>
      					@endcan
                @can('edit_review_questions')
      						<a href="`+goto+`" class="btn btn-sm blue" target="_blank" title="GoTo">
      			              <i class="fa fa-link"></i>
      			            </a>
      					@endcan

                @can('delete_review_questions')
{{--                @csrf--}}
